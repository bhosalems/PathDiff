...
class ConchEmbedder(nn.Module):
    def __init__(
        ...
        self.ckpt_path = ckpt
        self.device=device
        self.model, _ = create_model_from_pretrained(model_cfg, self.ckpt_path, force_image_size=256, device=device)
        self.tokenizer = get_tokenizer()
        self.model.eval()

    def forward(self, text):
        text_tokens = tokenize(texts=[text], tokenizer=self.tokenizer).to(self.device)
...
class MaskEmbedder(nn.Module):
    def __init__(self, conditional_channels=3, out_channels=256):
        super().__init__()
        
        self.conv_in = nn.Conv2d(conditional_channels, out_channels, kernel_size=3, padding=1, stride=2)
        self.conv_out = self.zero_module(
            nn.Conv2d(out_channels, conditional_channels, kernel_size=3, padding=1, stride=2)
        )
...
class ControlNetMaskEmbedder(nn.Module):

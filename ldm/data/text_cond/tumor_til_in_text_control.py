...
class TCGADataset(Dataset):
    """Dataset with tumor presence labels in text, the only difference from the original TCGADataset is that it returns the get_item with what controlnet wants"""
    ...
    def __getitem__(self, idx):
        ...
        return {
            "jpg": image,
            "txt": text_prompt,
            "hint":  np.zeros((256, 256, 3), np.uint8) # for classifier free guidance
        }

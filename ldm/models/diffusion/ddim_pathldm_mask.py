    ...
    def p_sample_ddim(
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            if sample_uncond_separate:
                    ...
                    for k in c:
                        if isinstance(c[k], list):
                            c_in[k] = [torch.cat([
                                ...
                                c[k][i]]) for i in range(len(c[k]))] # Uncodntional and conditional tokens/control are concatenated here
                        else:
                            c_in[k] = torch.cat([
                ...
                e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in).chunk(2) # this is just to process them together in on epass.
            e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond) # NOTE Mahesh- this is how the unconditional guidance scale is applied, 
            # and that is why uncontional tokens are also calculated in smapling or inference.

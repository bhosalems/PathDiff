...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        if self.updown:
            ...
            h = self.in_layers(x.to(torch.float32)) # mahesh remove the typecasting TODO
        emb_out = self.emb_layers(emb.to(torch.float32)).type(h.dtype) # mahesh remove the typecasting TODO
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
        return self.skip_connection(x.to(torch.float32)) + h # mahesh remove the typecasting TODO
